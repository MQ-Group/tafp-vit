...
class Attention(nn.Module):
    ...
    def __init__(self, layer_idx, feature_map_compssion_en, inter_layer_token_pruning_en, intra_block_row_pruning_en, 
                 dim, num_heads=8, head_dim=None, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
    ...
    def forward(self, x, first_compression_layer_idx=24, padding_mask=None):
        B, N, C = x.shape
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, -1, self.head_dim * self.num_heads)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class Block(nn.Module):
    ...
    '''
    def __init__(self, layer_idx, feature_map_compssion_en, inter_layer_token_pruning_en, intra_block_row_pruning_en,
                 dim, num_heads, head_dim=None, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = Attention(layer_idx, feature_map_compssion_en, inter_layer_token_pruning_en, intra_block_row_pruning_en,
    ...
    def forward(self, x, first_compression_layer_idx, padding_mask=None):
...
class PatchEmbedNaive(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size) # [B, 3, 224, 224] -> [B, embed_dim, 14, 14]
...
class PatchEmbed4_2(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.num_patches = num_patches
        self.embed_dim = embed_dim
        self.conv1 = nn.Conv2d(in_chans, 64, kernel_size=7, stride=2, padding=3, bias=False)  # [B, 3, 224, 224] ->[B. 64, 112, 112]
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=False)  # [B. 64, 112, 112] -> [B. 64, 112, 112]
        self.bn2 = nn.BatchNorm2d(64)
        self.conv3 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=False)  # [B. 64, 112, 112] -> [B. 64, 112, 112]
        self.bn3 = nn.BatchNorm2d(64)

        self.proj = nn.Conv2d(64, embed_dim, kernel_size=new_patch_size, stride=new_patch_size) # [B. 64, 112, 112] -> [B. embed_dim, 14, 14]
    def forward(self, x):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
...
class PatchEmbed4_2_128(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.conv1 = nn.Conv2d(in_chans, 128, kernel_size=7, stride=2, padding=3, bias=False)  # [B. 3, 224, 224] ->[B. 128, 112, 112]
        self.bn1 = nn.BatchNorm2d(128)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1, bias=False)  # [B. 128, 112, 112] -> [B. 128, 112, 112]
        self.bn2 = nn.BatchNorm2d(128)
        self.conv3 = nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1, bias=False)  # [B. 128, 112, 112] -> [B. 128, 112, 112]
        self.bn3 = nn.BatchNorm2d(128)

        self.proj = nn.Conv2d(128, embed_dim, kernel_size=new_patch_size, stride=new_patch_size) # [B. 128, 112, 112] -> [B. embed_dim, 14, 14]
    def forward(self, x):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
