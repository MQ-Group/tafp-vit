...
def load_pretrained_weights(model, checkpoint_path, use_ema=False, strict=True, num_classes=1000):
    ...
    ################ t
    # compressdecompress0 =['blocks.0.attn.compressdecompress.compress.weight', 'blocks.0.attn.compressdecompress.compress.bias', 'blocks.0.attn.compressdecompress.decompress.weight', 'blocks.0.attn.compressdecompress.decompress.bias']
    # for key in compressdecompress0:
    #     state_dict.pop(key)
    # model.load_state_dict(state_dict, strict=strict)
    ################ s
    # compressdecompressi =[['blocks.{}.attn.compressdecompress.compress.weight'.format(i), 'blocks.{}.attn.compressdecompress.compress.bias'.format(i), 'blocks.{}.attn.compressdecompress.decompress.weight'.format(i), 'blocks.{}.attn.compressdecompress.decompress.bias'.format(i)] for i in range(0, 8)]
    # for compressdecompress in compressdecompressi:
    #     for key in compressdecompress:
    #         state_dict.pop(key)
    # model.load_state_dict(state_dict, strict=strict)
