...
class LV_ViT(nn.Module):
    ...
    def forward(self, x, first_compression_layer_idx=24): # [B=64, 3, 224, 224]
        x = self.forward_embeddings(x) # conv based patch embedding # [B=64, C=240/384/512/768, 14, 14]
        ...
        if self.return_dense:  # True for lvvit_t lvvit_s lvvit_m lvvit_l
            ...
            if not self.training: # self.training is undefined
            ...
            return (x_cls, x_aux, (bbx1, bby1, bbx2, bby2)), fm_comp_loss
...
def lvvit_t(pretrained=False, **kwargs):
    model = LV_ViT(feature_map_compssion_en=False, inter_layer_token_pruning_en=False, intra_block_row_pruning_en=False,
