...
def inter_layer_token_pruning(attn, num_token_pruning):
    ...
    token_pruning_mask = token_rank >= num_token_pruning

    return token_pruning_mask

def reshape_attn(token_pruning_mask, num_token_pruning, attn):
    '''
    input:
        token_pruning_mask: (B, N)
        num_token_pruning: (1)
...
def reshape_x(token_pruning_mask, x):
    '''
    input:
        token_pruning_mask: (B, N)
        x: (B, N, C)
...
def intra_block_row_pruning(attn, num_block_row_pruning):
    ...
    attn = attn.permute(0, 1, 2, 4, 3, 5).contiguous().view(B, H, (N+63)//64*64, (N+63)//64*64)
